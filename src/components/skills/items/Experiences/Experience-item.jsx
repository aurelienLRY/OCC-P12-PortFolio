export const Gerant = () => (
  <div className="experience-item">
    <h3 className="experience-item_title">Cofondateur - Gérant SCOP</h3>
    <h4 className="experience-item_subTitle">Solution de réalité virtuelle</h4>
    <p className="experience-item_content">
      Diriger une coopérative spécialisée en réalité virtuelle, développant des
      compétences en gestion de projet et en développement d'applications
      innovantes.
    </p>
    <p className="experience-item_hashtag"> #R&D #InnovationTechnique #GestionDeProjet #RéalitéVirtuelle</p>
  </div>
);
export const ExploitantDrone = () => (
  <div className="experience-item">
    <h3 className="experience-item_title">Exploitant drone - Télépilotte</h3>
    <h4 className="experience-item_subTitle">
      Spécialiste des prises de vues aériennes
    </h4>
    <p className="experience-item_content">
      Gestionnaire de flotte de drones, assurant la conformité du matériel et la sécurité des missions. Prototypage de drone, mise en conformité.
    </p>
    <p className="experience-item_hashtag">
      #Captation #GestionDeProjet #InnovationTechnique
    </p>
  </div>
);
export const DroneLent = () => (
  <div className="experience-item">
    <h3 className="experience-item_title">Chef d’équipe drone lent</h3>
    <h4 className="experience-item_subTitle">Renseignement militaire</h4>
    <p>
      Responsable d’une équipe dans le renseignement d’origine image, occupant des fonctions variées telles que pilote de drone et analyste d'images.   
    </p>
    <p className="experience-item_hashtag">
    #AnalyseDeDonnées #GestionOpérationnelle #Leadership
    </p>
  </div>
);
export const Dem = () => (
  <div className="experience-item ">
    <h3 className="experience-item_title">Technicien DEM</h3>
    <h4 className="experience-item_subTitle">Militaire</h4>
    <p>
      Maintenance des équipements électromagnétique, développant une rigueur technique et une
      attention aux détails.
    </p>
    <p className="experience-item_hashtag">
    #Rigueur #Fiabilité #Détermination
    </p>
  </div>
);
